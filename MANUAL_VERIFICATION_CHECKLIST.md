...
## 918_DefectEngine — pending real verification (added 2026-08-16, not yet run)
`918_DefectEngine.js` (Phase 2/3, PropertyCase + DefectItem lifecycle) has 61/61 passing against the project's own Node `GasShim.js` — see `local_precheck_test_918.js`, including a real precedence bug the shim run caught and fixed (see ADR-P15). That confirms the *logic*. It does **not** confirm real GAS behavior, same "shim vs. real" gap this checklist tracked for the Obligation Engine before its 2026-07-29 real run — 918 is at that same starting point now.
- [x] `Properties` sheet migration (`DevelopmentName`/`UnitLabel` appended at columns AD/AE, ADR-P17) — **confirmed 2026-08-16**, CC added the two header cells by hand, then ran `runAllPropertyAssetEngineTests` (21/21) and `runAllPropertyOSTests` (141/141), both green.
- [ ] `runAllPropertyOSTests()` still 141/141 with 918/903 deployed alongside — no regression in the existing engines from the new global names/sheets. (Next action, per CC 2026-08-16 — deploy, then run this before Phase 4.)
- [ ] `withDefectEngineLock_`'s real `LockService.getScriptLock()` — same open questions as the Concurrency section above (30s timeout reasonableness, real contention), now also for 918's own lock. It's a *separate* acquisition from 912's own lock (never nested — each Command acquires exactly one), but that's reasoned, not yet exercised for real.
- [ ] `getCachedDefectEngineCommandResult_`/`cacheDefectEngineCommandResult_` — real within-execution cache hit, same check 991/993 already did for Obligation. TTL *expiry* has the same unconfirmed status as the Caching section above.
- [ ] The 8 new sheets (`PropertyCases`, `DefectItems`, `DailyProgressChecks`, `Correspondences`, `RectificationEvents`, `SecondaryDamages`, `PropertyCaseTimeline`, `Evidence`) get created cleanly by `initDefectEngineSchema_()`/`initDocumentEngineSchema_()` on the real Spreadsheet — headers frozen, date columns forced to `@`, same three checks the Sheets behavior section above already closed out for the older sheets.
- [ ] `appendCaseTimelineEntry_` + `publishPropertyEvent_` sharing one try block, on a real throw from the latter — does `logDefectEnginePartialFailure_` fire and read clearly in real Stackdriver logs, the way the Obligation Engine's equivalent was confirmed to on 2026-07-29 (Platform-level section above)? Same mechanism, not yet exercised for 918 specifically.
- [ ] Real-world timing for a realistic batch of `addDefectItem` calls (CC's real case starts at 140+ defects) — each Command is a handful of Sheets API calls with no loop over existing data, so should be fast; "should be" isn't "confirmed."
